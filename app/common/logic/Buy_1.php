...
class Buy_1 {
    ...
    public function updatePintuan($post_data, $goods_info, $order, $order_type, $member_id) {
        ...
        if ($pintuangroup_id == 0) {
            ...
            $res['pintuangroup_id'] = $pintuangroup_id;
            //拼团统计新增开团数量
            Db::name('ppintuan')->where('pintuan_id', $pintuan_id)->inc('pintuan_count')->update();
        ...
        //开团统计新增人数
        Db::name('ppintuangroup')->where('pintuangroup_id', $pintuangroup_id)->inc('pintuangroup_joined')->update();
        //下单后清除缓存
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = round($order_info['rcb_amount'] + $order_amount, 2);
                ...
                if (!isset($order_info['order_goods'])) {
                    $order_list[$key]['order_goods'] = $order_info['order_goods'] = $order_model->getOrdergoodsList(array(array('order_id', '=', $order_info['order_id'])));
                }
                ...
                if ($available_rcb_amount > 0) {
                    ...
                    //支付金额保存到订单
                    $data_order = array();
                    $order_list[$key]['rcb_amount'] = $data_order['rcb_amount'] = round($order_info['rcb_amount'] + $available_rcb_amount, 2);
                    $result = $order_model->editOrder($data_order, array('order_id' => $order_info['order_id']));
...
//                 if (empty($pd_payment_info)) return;
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $data_order['payment_time'] = TIMESTAMP;
                $data_order['payment_code'] = 'predeposit';
                $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = round($order_info['pd_amount'] + $order_amount, 2);
                $result = $order_model->editOrder($data_order, array('order_id' => $order_info['order_id']));
                if (!$result) {
                    throw new \think\Exception('订单更新失败', 10006);
                }
                ...
                if ($available_pd_amount > 0) {
                    ...
                    //预存款支付金额保存到订单
                    $data_order = array();
                    $order_list[$key]['pd_amount'] = $data_order['pd_amount'] = round($order_info['pd_amount'] + $available_pd_amount, 2);
                    $result = $order_model->editOrder($data_order, array('order_id' => $order_info['order_id']));
